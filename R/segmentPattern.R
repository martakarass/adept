...
segmentPattern <- function(x,
  ...
  if (!(all(is.numeric(x)) & is.vector(x))) stop("x must be a numeric (vector) vector.")
  if (!(length(x.fs) == 1 & is.numeric(x.fs) & x.fs > 0 & is.vector(x.fs))) stop("x.fs must be a positive numeric scalar.")
  template.cond1 <- all(is.numeric(template)) & is.vector(template)
  template.cond2 <- is.list(template) & all(sapply(template, function(vec) all(is.numeric(vec)) & is.vector(vec)))
  if (!(template.cond1 || template.cond2)) stop("template must be a numeric (vector) vector, or a list of numeric (vector) vectors.")
