...
Simulate a time-series `x`; assume that `x` is collected frequency of 100 Hz, there is one pattern of a fixed duration 1.01 seconds present in `x`, and there is no noise in collected data. 
...
Segment pattern from data. Assume perfect template is available. Use grid of potential pattern durations of {0.9, 1, 1.1} seconds.
...
Assume grid of potential pattern durations of {0.9, 1.01, 1.1} seconds (the grid is now "perfect" in a sense it contains duration of the true pattern used in data simulation). 
...
Simulate a time-series `x`; assume that time-series is collected frequency of 100 Hz, there are two patterns of various duration present in `x`, and there is noise in collected data. 
...
Assume perfect grid of potential pattern duration, {0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3} seconds. Segment `x`. 
...
Segment `x2`; smooth data time-series before similarity matrix computation.  Assume more dense grid of potential pattern duration. 
