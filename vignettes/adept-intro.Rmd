...
We next generate a dense grid of potential pattern durations, including value `101` used in the `x` simulation to obtain a perfect match (`sim_i = 1`).
...
Use two templates simultaneously in segmentation. We observe that the index of a pattern template best matched to a pattern in the time-series `x` is `1` and `2` interchangeably.
