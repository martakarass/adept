...
library(adept)
library(lubridate)
data = adept::acc_running
data$date_time = force_tz(data$date_time, tzone = "UTC")
head(data)
```
...
t1 <- as.POSIXct("2018-10-25 18:00:00", tz = "UTC") 
t2 <- as.POSIXct("2018-10-25 18:10:00", tz = "UTC") 
t3 <- as.POSIXct("2018-10-25 18:22:00", tz = "UTC") 
...
if (nrow(df) > 0) {
  ...
  msg = paste0("T1: ", t1,
               ", dt_row1 of acc_running_sub: ",
               data$date_time[1], " ac_running_sub has no rows!\n")
  stop(msg)
}
```
...
data.frame(vmc = vmc.vec,
           date_time = data$date_time[rn.seq],
           sensor_location = data$sensor_location[rn.seq]) %>%
  filter(date_time < max(data$date_time) - 5, 
         date_time > min(data$date_time) + 5) %>%
...
(vmc.low.frac <- mean(vmc.vec[data$sensor_location[rn.seq] == "left_ankle"] < 0.5))
```
...
data$vm_smoothed1 <- windowSmooth(x = data$vm, x.fs = 100, W = 0.05)
...
x.la <- data$vm[data$sensor_location == "left_ankle"]
...
template <- list(stride_template$left_hip[[1]][1, ],
                 stride_template$left_hip[[2]][2, ])
x.lh <- data$vm[data$sensor_location == "left_hip"]
