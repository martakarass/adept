...
* The knitr HTML output truncates the subsecond level accurracy of `POSIXct` values stored in `date_time` column, hence in first rows of the table we see `2018-10-25 17:57:30` only; when `head(acc_running)` is executed in the `R` console, the result would be likely `2018-10-25 17:57:30.00`, `2018-10-25 17:57:30.01` etc. 
...
Time collection spans 25 minutes. It agrees with a number of observations per sensor we have (`150000`), given the data was colected at frequency 100 observations per second. 
```{r}
...
One way to visualize raw accelerometry data is to plot it as a three-dimmensional time-series. Here, we plot data from three different time frames, each of 4 seconds length, simultaneously for data collected at left ankle and right ankle. 
...
Using vector magnitude $(vm)$ is a way to summarize a three-dimmensional time-series $(x,y,z)$ as a one-dimensional time-series. Vector magnitude is computed as $vm = \sqrt{x^2 + y^2 + z^2}$ at each time point of data collection. 
...
1. In computation of similarity matrix between pattern template and (possibly smoothed) $(vm)$ signal. Here, `x.adept.ma.W` argument of `segmentPattern` function defines a length of a window used in moving average smoothing; it is expressed in seconds. 
    ...
    For this phase, we recomend low/moderarte smoothing (`x.adept.ma.W = 0.05`, `x.adept.ma.W = 0.15`) so as tiny $(vm)$ bumps are smoothed, but the pattern shape feaftures are not lost. 
...
We perform strides segmentation from $(vm)$ accelerometry signal. See `?segmentPattern` for detailes explanation of parameters. 
...
* `tau_i` - index of a vector (here: $(vm)$ time-series) where an identified pattern occurence starts,
* `T_i` - duration of identified pattern occurence, expressed in a vector length,
* `sim_i` - value of similarity statistic between an identified pattern occurence and corresponding window of a time-series used in similarity matrix computation (see `?segmentPattern` for details),
* `template_i` - if `compute.template.idx` euqlas `TRUE`: index of pattern template that yielded highest similarity for an identified pattern occurrence; otherwise: `NA`.
...
We now plot estimated stride duration in seconds (`T_i`/100) aganist estimated  stride start in minutes (`tau_i`/(100*60)). Note: 
...
2. Within each of three $(vm)$ fragments, use a function to automatically idenitfy *all* local maxima.

3. Within each of three $(vm)$ fragments, indentify a *subset* of local maxima that corresponds to $(vm)$ peaks of interest via visual inspection ("manual" part). 

4. For each of three $(vm)$ fragments, cut the $(vm)$ fragment into parts at the points of indentified local maxima subset. Interpolate parts to common a length and compute their point-wise average. The resulted point-wise average is a newly created pattern template. 
